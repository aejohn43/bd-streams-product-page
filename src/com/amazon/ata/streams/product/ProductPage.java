...
public class ProductPage {
    ...
    public Optional<ProductV2.BuyingOption> getFirstBuyingOption() {
        ...
        return productV2.buyingOptions()
                .stream()
                .findFirst();
    }
    ...
    public Optional<String> extractLookImageUrl(Integer longestDimension) {
        ...
        return productV2.productImages()
                ...
                .stream()
                .filter(Objects::nonNull)
                .filter(image -> {return LOOK_VARIANT.equals(image.variant());})
                .map(image -> extractImageUrl(image, longestDimension))
                .filter(Objects::nonNull)
                .findFirst();
    }
    ...
    public List<ProductV2> getSimilarProducts(final SortByEnum sortBy,
        ...
        return Optional.ofNullable (productV2.getSimilarProducts()) // 3: return and 2 methods
                .orElse(Collections.emptyList()) // 1 method
                .stream() // 1: method
                .filter(Objects::nonNull) // 1: method
                .filter(ProductV2::isValid) // 1: method
                .filter(product -> priceRange. priceIsWithin (product.getPrice())) // 2 2 methods
                .filter(product -> product.getShippingPrograms ().stream() // 2: 2 method calls
                        .anyMatch (primeOption::matches)) // 2: 2 methods
                .sorted (comparatorForSortBy.getOrDefault(sortBy, passthroughComparator())) // 3: 3 methods
                .collect(Collectors.toList()); // 2: 2 methods
    }
